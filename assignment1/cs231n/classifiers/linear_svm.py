...
def svm_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            #correct_class_score의 shape는 [1,]
            if margin > 0:
                loss += margin
                # Incorrect class에 대해서는 input 이미지의 픽셀값만큼 더해준다. 
                dW[:, j] += X[i].T
                
                # Correct class에 대해서는 loop를 돌 때마다 input image의 픽셀값만큼 빼준다. 
                dW[:, y[i]] += -X[i].T               
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    num_train = X.shape[0]
    scores = np.dot(X, W) # shape of (N, C)
    correctlabel_scores = scores[np.arange(num_train), y].reshape(num_train, 1) # (N, )
    margins = np.maximum(0, scores-correctlabel_scores + 1)
    margins[np.arange(num_train), y] = 0 # (N, C)
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    margins[margins > 0] = 1
    ones_count = np.sum(margins, axis =1)
    margins[np.arange(num_train), y] -= ones_count
    # X : (N, D)
