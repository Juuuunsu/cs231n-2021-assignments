...
def svm_loss(x, y):
    ...
    N = x.shape[0]
    correctlabel_scores = x[np.arange(N), y] # N, 1
    margins = np.maximum(0, x - correctlabel_scores[:, np.newaxis] + 1) # (N, C)
    margins[np.arange(N), y] = 0
    loss = np.sum(margins) / N 

    dx = np.zeros_like(x) # np.zeros_like(x) : x의 shape와 같은 0으로 가득찬 array를 반환
    dx[margins > 0] = 1
    ones_count = np.sum(margins>0, axis =1)
    dx[np.arange(N), y] -= ones_count
    dx /= N
