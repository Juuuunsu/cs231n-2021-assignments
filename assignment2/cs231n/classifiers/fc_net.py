...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        # input, hidden, output layer를 hstack
        # NOTE input layer는 총 layer 수에 포함되지 않음. index 1부터 시작.
        layer_dims = np.hstack([input_dim, hidden_dims, num_classes])

        for l in range(self.num_layers):  # Loop for l = 0 ~ L-1
            self.params["W" + str(l + 1)] = weight_scale * np.random.randn(
                layer_dims[l], layer_dims[l + 1]
            )  # W1 ~ WL
            self.params["b" + str(l + 1)] = np.zeros(
                layer_dims[l + 1]
            )  # B1 ~ BL
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers - 1):
            ...
            if self.normalization:
                ...
                X, cache = affine_norm_relu_forward(
                    X, W, b, gamma, beta, bn_params, self.normalization
                ) # X as new input for next layer

            else:  # no batch/layer norm
        ...
        # (2) {affine - [batch/layer norm] - relu - [dropout]} x (L - 1) - affine
